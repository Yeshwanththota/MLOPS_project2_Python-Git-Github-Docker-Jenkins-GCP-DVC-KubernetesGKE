...
class ModelTraining:
    def __init__(self,data_path):
        ...
        self.experiment = comet_ml.Experiment(
            api_key="QV2mcMGURiGydwgrDMYKywNXa",
            project_name="mlops-project2",
            workspace="yeshwanththota",
        )
        logger.info("Comet ML experiment initialized")
        logger.info("ModelTraining initialized with data path: %s", data_path)
    def load_data(self):
        try:
    ...
    def train_model(self):
        try:
            x_train_array, y_train, x_test_array, y_test = self.load_data()
            ...
            raise CustomException(f"Error in train_model: {e}", e)
    def extract_weights(self,layer_name,model):
        try:
            ...
            logger.info("Weights extracted successfully from layer: %s", layer_name)
            return weights
        except Exception as e:
            logger.error(f"Error extracting weights from layer {layer_name}: {e}")
            raise CustomException(f"Error extracting weights from layer {layer_name}: {e}", e)
    def save_model_weights(self,model):
        try:
            ...
            logger.info("Weights saved successfully at %s and %s", USER_WEIGHTS_PATH, ANIME_WEIGHTS_PATH)
        except Exception as e:
            logger.error(f"Error saving model weights: {e}")
            raise CustomException(f"Error saving model weights: {e}", e)
